...
function simColor(s: number) { return s >= 0.7 ? "#00C805" : s >= 0.5 ? "#FFC107" : "#FF9800"; }
function retColor(v: number) { return v >= 0 ? "#00C805" : "#FF5722"; }
...
export default function DoppelgangerTab() {
  ...
  if (err) return <Card title="Doppelganger"><div className="text-sm text-[#7C879B]">Analog database unavailable.</div></Card>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-end gap-3">
        <div className="relative">
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={`Ticker (current: ${row.ticker})`}
            className="w-56 rounded-md border border-[#1E2632] bg-[#121723] px-3 py-2 text-sm text-white" />
          {suggestions.length > 0 && (
            <div className="absolute z-20 mt-1 w-full overflow-hidden rounded-md border border-[#1E2632] bg-[#0F1420] shadow-xl">
              {suggestions.map((s) => (
                <button key={s.ticker} onClick={() => { selectTicker(s.ticker); setQuery(""); }} className="block w-full px-3 py-1.5 text-left text-sm hover:bg-[#1B2433]">
        ...
        <label className="text-xs text-[#9CA7BB]">Match mode
          <select value={sectorMode} onChange={(e) => setSectorMode(e.target.value)} className="mt-1 block rounded-md border border-[#1E2632] bg-[#121723] px-2 py-1.5 text-sm text-white">
        ...
        <label className="text-xs text-[#9CA7BB]">Tag
          <select value={tag} onChange={(e) => setTag(e.target.value)} className="mt-1 block rounded-md border border-[#1E2632] bg-[#121723] px-2 py-1.5 text-sm text-white">
            <option>All</option>{db.tags.map((t) => <option key={t}>{t}</option>)}
          </select>
        </label>
        <label className="flex items-center gap-2 text-xs text-[#C3CAD7]"><input type="checkbox" checked={dedupe} onChange={(e) => setDedupe(e.target.checked)} /> Dedupe eras</label>
      </div>
      ...
      {matches.length === 0 ? (
        <Card title="No analogues"><div className="text-sm text-[#7C879B]">No historical analogues above the similarity threshold for {row.ticker} with these filters. Try "Any sector".</div></Card>
      ) : (
        <>
          {agg && (
            <Card title="Aggregate forward-return forecast" sub={`Similarity-weighted across ${agg.contributing_count} analogues`}>
              ...
              <div className="mt-2 text-xs text-[#7C879B]">1yr range: {fmtPct(agg.worst_1yr, 0, true)} … {fmtPct(agg.best_1yr, 0, true)} · median {fmtPct(agg.median_1yr_pct, 1, true)}</div>
            </Card>
          )}
            ...
            return (
              <Card key={m.match_key} title={`${m.company} — ${m.era}`} sub={`${m.sector}${m.same_sector ? " · same sector" : ""}`}>
                <div className="mb-2 flex items-center gap-3">
                  <div className="h-2 flex-1 overflow-hidden rounded-full bg-[#1A2130]">
                ...
                {fr && (
                  <div className="mb-2 grid grid-cols-3 gap-2 text-sm">
                    {(["1yr", "3yr", "5yr"] as const).map((h) => (
                      <div key={h} className="rounded border border-[#1E2632] px-2 py-1">
                        <div className="text-[10px] uppercase text-[#7C879B]">{h} after</div>
                        <div className="font-semibold" style={{ color: retColor(fr[h]) }}>{fmtPct(fr[h], 0, true)}</div>
                      </div>
                    ))}
                  </div>
                )}
                {m.narrative && <p className="text-sm text-[#C3CAD7]">{m.narrative}</p>}
                {m.outcome && <p className="mt-1 text-xs text-[#9CA7BB]"><strong className="text-[#C3CAD7]">Outcome:</strong> {m.outcome}</p>}
                {m.lesson && <p className="mt-1 text-xs text-[#9CA7BB]"><strong className="text-[#C3CAD7]">Lesson:</strong> {m.lesson}</p>}
                <div className="mt-2 flex flex-wrap gap-1">{m.tags.map((t) => <span key={t} className="rounded-full bg-[#1A2130] px-2 py-0.5 text-[10px] text-[#9CA7BB]">{t}</span>)}</div>
              </Card>
            );
          })}
        </>
      )}
      <p className="text-[10px] text-[#5C6678]">Historical analogues are illustrative pattern-matches, not predictions. Past performance does not indicate future results.</p>
    </div>
  );
}
