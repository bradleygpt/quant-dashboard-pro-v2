...
export default function PortfolioTab() {
  ...
  const holdingCols = useMemo<Column<Position>[]>(() => [
    { key: "ticker", header: "Ticker", sortValue: (p) => p.ticker,
      render: (p) => <button onClick={() => goToDetail(p.ticker)} className="font-semibold text-link hover:underline">{p.ticker}</button> },
    ...
    { key: "gain", header: "Gain", align: "right", sortValue: (p) => p.gain_pct,
      render: (p) => <span style={{ color: p.gain_pct == null ? INK.mute : p.gain_pct >= 0 ? SEM.pos : SEM.neg }}>{p.gain_pct == null ? "—" : fmtPct(p.gain_pct, 1, true)}</span> },
    { key: "rating", header: "Rating", sortValue: (p) => RATING_RANK[p.rating] ?? 0, render: (p) => <RatingBadge rating={p.rating} /> },
    { key: "remove", header: "", sortable: false, render: (p) => <button onClick={() => setHoldings(holdings.filter((h) => h.ticker !== p.ticker))} className="text-xs text-mute hover:text-red-400">remove</button> },
    ...
    <div className="space-y-4">
      ...
      <Card title="Add holding" sub="Stored locally in your browser (localStorage). Not uploaded anywhere.">
        <div className="flex flex-wrap items-end gap-2">
          ...
          {holdings.length > 0 && <button onClick={() => setHoldings([])} className="rounded-md border border-line px-3 py-1.5 text-sm text-ink-3 hover:bg-hover">Clear all</button>}
        </div>
        <div className="mt-1 text-[10px] text-dim">CSV: a header row with Symbol/Ticker, Quantity/Shares, and optional Cost Basis Per Share (Fidelity-style export works).</div>
      </Card>

      {!analysis ? (
        <div className="text-sm text-mute">Add holdings to see portfolio analytics.</div>
      ) : (
        <>
          ...
          <Card title="Holdings">
            ...
            {analysis.unmatched_tickers.length > 0 && <div className="mt-2 text-xs text-warn-hot">Unmatched (not in universe): {analysis.unmatched_tickers.join(", ")}</div>}
          </Card>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {Object.entries(analysis.factor_tilts).map(([p, f]) => (
                    <tr key={p} className="border-t border-line-faint">
                      <TD className="text-ink-2">{p}</TD><TD className="text-right">{f.portfolio.toFixed(1)}</TD>
                      <TD className="text-right text-ink-3">{f.universe.toFixed(1)}</TD>
                      <TD style={{ color: f.tilt === "Overweight" ? SEM.pos : f.tilt === "Underweight" ? SEM.neg : INK.ink3 }}>{f.tilt}</TD>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody>
                  {Object.entries(analysis.sector_weights).map(([sector, s]) => (
                    <tr key={sector} className="border-t border-line-faint">
                      <TD className="text-ink-2">{sector}</TD><TD className="text-right">{fmtPct(s.weight * 100)}</TD>
                      <TD className="text-right text-ink-3">{s.count}</TD><TD className="text-right">{s.avg_score ? s.avg_score.toFixed(2) : "—"}</TD>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <Card title="Pillar Tilt — Portfolio vs Universe">
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={pillarTilt} margin={{ top: 5, right: 10, bottom: 0, left: 0 }}>
                <XAxis dataKey="pillar" tick={{ fill: INK.ink3, fontSize: 11 }} />
                <YAxis domain={[0, 12]} tick={{ fill: INK.mute, fontSize: 11 }} width={28} />
          ...
          <Card title="🎲 Monte Carlo Simulation" sub="Geometric Brownian Motion with mean reversion, return caps, 0.45 cross-correlation, and macro scenario adjustment.">
            <div className="mb-3 flex flex-wrap items-end gap-3">
              <label className="text-xs text-ink-3">Sims
                <select value={mcSims} onChange={(e) => setMcSims(+e.target.value)} className="mt-1 block rounded-md border border-line bg-head px-2 py-1.5 text-sm text-white">
                  {[1000, 5000, 10000].map((n) => <option key={n} value={n}>{n.toLocaleString()}</option>)}
                </select>
              </label>
              <label className="text-xs text-ink-3">Horizon
                <select value={mcHorizon} onChange={(e) => setMcHorizon(+e.target.value)} className="mt-1 block rounded-md border border-line bg-head px-2 py-1.5 text-sm text-white">
                  {[63, 126, 252].map((n) => <option key={n} value={n}>{HORIZON_LABEL[n]}</option>)}
                </select>
              </label>
              <label className="text-xs text-ink-3">Scenario
                <select value={mcScenario} onChange={(e) => setMcScenario(e.target.value as Scenario)} className="mt-1 block rounded-md border border-line bg-head px-2 py-1.5 text-sm text-white">
                  {(["Blended", "Bull", "Base", "Bear"] as Scenario[]).map((s) => <option key={s} value={s}>{s}</option>)}
                </select>
              </label>
              <span className="text-[11px] text-mute">Bull +8% drift · Base neutral · Bear −12% · Blended 25/50/25.</span>
            </div>
            {!mc ? <div className="py-4 text-sm text-mute">Add priced holdings to run a simulation.</div> : (
              <>
                ...
                <ResponsiveContainer width="100%" height={340}>
                  <AreaChart data={mc.fan.map((f) => ({ day: f.day, p50: f.p50, base: f.p5, outer: f.p95 - f.p5, base2: f.p25, inner: f.p75 - f.p25 }))} margin={{ top: 12, right: 12, bottom: 0, left: 4 }}>
                    ...
                    <XAxis dataKey="day" type="number" domain={[0, mc.horizonDays]} tick={{ fill: INK.mute, fontSize: 11 }} tickFormatter={(d) => `${d}d`} minTickGap={36} />
                    ...
                    <Area type="monotone" dataKey="inner" stackId="i" stroke="none" fill={ASSET.eth} fillOpacity={0.20} isAnimationActive={false} name="25–75th" />
                    <Area type="monotone" dataKey="p50" stroke={ASSET.eth} strokeWidth={2} fill="none" dot={false} name="Median" isAnimationActive={false} />
                  </AreaChart>
                </ResponsiveContainer>
                <div className="text-[10px] text-dim">Shaded = 5th–95th and 25th–75th percentile cone; line = median portfolio value over time.</div>

                <div className="mt-3 grid grid-cols-1 gap-4 lg:grid-cols-2">
                  {/* Outcome probabilities (6 rows) */}
                  <div>
                    <div className="mb-1 text-xs font-semibold uppercase text-mute">Outcome Probabilities</div>
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {([
                          ...
                          <tr key={l} className="border-t border-line-faint"><TD className="text-ink-2">{l}</TD><TD className="text-right">{v.toFixed(1)}%</TD></tr>
                        ))}
                      </tbody>
                    </table>
                  </div>
                  ...
                  <div>
                    <div className="mb-1 text-xs font-semibold uppercase text-mute">Value at Risk (terminal)</div>
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {([["5th (worst)", mc.percentiles.p5], ["25th", mc.percentiles.p25], ["50th (median)", mc.percentiles.p50], ["75th", mc.percentiles.p75], ["95th (best)", mc.percentiles.p95]] as const).map(([l, v]) => (
                          <tr key={l} className="border-t border-line-faint"><TD className="text-ink-2">{l}</TD><TD className="text-right">{fmtMoney(v, 0)}</TD>
                            <TD className="text-right" style={{ color: v >= mc.totalValue ? SEM.pos : SEM.neg }}>{fmtPct((v / mc.totalValue - 1) * 100, 1, true)}</TD></tr>
                        ))}
                      </tbody>
                    </table>
                  </div>
                ...
                <details className="mt-3 text-xs">
                  <summary className="cursor-pointer text-mute">Model Assumptions (per holding)</summary>
                  <table className="mt-2 w-full text-sm">
                    ...
                    <tbody>
                      {mc.holdingDetails.map((d) => (
                        <tr key={d.ticker} className="border-t border-line-faint"><TD className="text-ink-2">{d.ticker}</TD><TD className="text-right">{d.expReturnPct.toFixed(1)}%</TD><TD className="text-right">{d.volPct.toFixed(1)}%</TD><TD className="text-right">{d.weightPct.toFixed(1)}%</TD></tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="mt-1 text-[10px] text-dim">Per-holding return/vol derived from baked price-series momentum (1m/3m/6m/12m) blended with a long-term premium; not a forecast.</div>
              </>
            )}
          </Card>
        </>
